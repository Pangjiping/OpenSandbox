...
func startFleetMitmproxyIfEnabled() (*mitmTransparent, error) {
	...
	extra := strings.TrimSpace(os.Getenv(constants.EnvMitmproxyExtraPorts))
	dportsStr, _, err := parseMitmproxyPorts(extra)
	if err != nil {
...
// parseMitmproxyPorts validates the extra-ports env and returns the iptables
// --dports string (80,443 or 80,443,<extra>) plus the numeric list used by the
// nft interception rules. "80,443" is the base intercept set.
func parseMitmproxyPorts(extra string) (string, []int, error) {
	base := []int{80, 443}
	var all []int
	all = append(all, base...)
	if strings.TrimSpace(extra) != "" {
		for _, raw := range strings.Split(extra, ",") {
			...
			if err != nil || p <= 0 || p > 65535 {
				return "", nil, fmt.Errorf("invalid extra port %q", strings.TrimSpace(raw))
			}
			all = append(all, p)
		}
	}
	...
	return strings.Join(parts, ","), all, nil
}
