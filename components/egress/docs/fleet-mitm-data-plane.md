...
```
 ...
 └─────────────────────┘               │         ↓ (local delivery)        │
                                       ...
                                       │  active socket 127.0.0.1:18082/_active
                                       └──────────────────────────────────┘
```
...
- **Deliberate placement change vs the original design**: the original design
  ...
  1. The sandbox-netns OUTPUT filter (`pkg/sandboxnft`) and the Pod-netns
     forward hook see the **real destination** — policy matching stays exact
     with no rule changes (a sandbox-side DNAT would rewrite the dst before
     every filter hook saw it, breaking both layers' daddr matching).
